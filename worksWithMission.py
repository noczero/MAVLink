...
def arm_and_takeoff(altitude):
    ...
    while not myVehicle.armed: time.sleep(1)

    print("Taking Off")
    myVehicle.simple_takeoff(altitude)

    while True:
        v_alt = myVehicle.location.global_relative_frame.alt
...
print('Connecting...')
myVehicle = connect('udp:127.0.0.1:14551')
...
while True:
    ...
    if mode == 'GROUND':
        ...
        n_WP, missionList = get_current_mission(myVehicle)
        time.sleep(2)
        if n_WP > 0:
        ...
        add_last_waypoint_to_mission(myVehicle, myVehicle.location.global_relative_frame.lat,
                                     myVehicle.location.global_relative_frame.lon,
                                     myVehicle.location.global_relative_frame.alt)
        ...
        ChangeMode(myVehicle, "AUTO")

        # -- Change mode, set the ground speed
        myVehicle.groundspeed = gnd_speed
        mode = 'MISSION'
        ...
        print ("Current WP: %d of %d " % (myVehicle.commands.next, myVehicle.commands.count))
        if myVehicle.commands.next == myVehicle.commands.count:
            print ("Final waypoint reached: go back home")
            # -- First we clear the flight mission
            clear_mission(myVehicle)
            print ("Mission deleted")

            # -- We go back home
            ChangeMode(myVehicle, "RTL")
            mode = "BACK"

    elif mode == "BACK":
        if myVehicle.location.global_relative_frame.alt < 1:
